...
- Ausführlichere Erklärung: kurzer Einleitungssatz ("Setze die Nadel dahin, wo du gerade stehst: zwischen Denken und Fühlen, und zwischen angespannt und entspannt.") als normaler `.lead`-Text, darunter ein **2×2-Raster** (`.compass-explain-grid`) mit je einer kurzen Karte für Denken/Fühlen/Angespannt/Entspannt — spiegelt bewusst die zwei Achsen des Kompasses selbst. Die Karten haben volle Deckkraft (`#eef6f2`, gleicher Ton wie `.trial-box`) statt Transparenz — eine durchscheinende Glas-Variante (`rgba(255,255,255,.55)` + Blur) war je nach Fotostelle dahinter schlecht lesbar. Label und Text stehen einzeilig ("Mehr Denken: Kopf aktiv, viele Gedanken.", per CSS-`::after`-Doppelpunkt statt Zeilenumbruch nach dem Label) statt Label+Umbruch+Text — spart eine Zeile pro Karte. Abschlusssatz ("Daraus finden wir deine passende Meditation.") wieder als `.lead`. Alle Abstände bleiben grosszügig (keine zusammengequetschten Ränder) — das wirkte in einer Zwischenversion billig. "Zurück"/"Weiter" sind bewusst **nicht** anders gestylt als auf jeder anderen Seite (unterschiedliche Knopfgrössen je Seite wirkten inkonsistent) — die nötige Höhe wird stattdessen beim Kompass eingespart: 200px auf normalen Handys (iPhone 14, Pro Max) statt dem App-Standard 320px, auf kurzen Bildschirmen (`@media (max-height:700px)`, z. B. iPhone SE) ein deutlich kleinerer Kompass (115px) plus engere Abstände überall. Ziel und aktueller Stand: **alles inklusive der Buttons ganz unten bleibt ohne Scrollen sichtbar**, auf allen drei getesteten Bildschirmgrössen — kein Scrollen wird hier bewusst in Kauf genommen, auch wenn der Kompass auf sehr kleinen Bildschirmen dafür deutlich kleiner ausfällt.
...
- **Dauer statt Anzahl, schon auf der Kompass-Seite gewählt:** Statt "Wie viele Meditationen?" (1/2/3) gibt es nur eine Frage — "Wie lange möchtest du meditieren?" mit festen Werten **5/10/15/20/30 Minuten**. Dieser Picker sitzt auf der **Kompass-Seite** (`#durationOptsV2`, direkt vor "Weiter zu den Meditationen"), nicht mehr auf der Kategorie-Seite von Variante 2 selbst — beim Öffnen einer Kategorie ist die Zieldauer also schon gesetzt. Die Anzahl der ausgewählten Übungen ergibt sich automatisch aus der Zieldauer, keine separate Eingabe nötig.
