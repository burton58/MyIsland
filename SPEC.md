...
- Karten-Überschrift **"🧭 Der Kompass"** deutlich größer/fetter als der Standard-Kartentitel (`#compassPage`-Override: 1.55rem/700 statt 1.2rem/normal, stärkerer Schein) — eine erste, dezentere Version ging auf dem hellen Foto optisch unter.
- Ausführlichere Erklärung: kurzer Einleitungssatz ("Setze die Nadel dahin, wo du gerade stehst: zwischen Denken und Fühlen, und zwischen angespannt und entspannt.") als normaler `.lead`-Text, darunter ein **2×2-Raster** (`.compass-explain-grid`) mit je einer kurzen Karte für Denken/Fühlen/Angespannt/Entspannt — spiegelt bewusst die zwei Achsen des Kompasses selbst. Die Karten haben volle Deckkraft (`#eef6f2`, gleicher Ton wie `.trial-box`) statt Transparenz — eine durchscheinende Glas-Variante (`rgba(255,255,255,.55)` + Blur) war je nach Fotostelle dahinter schlecht lesbar. Abschlusssatz ("Daraus finden wir deine passende Meditation.") wieder als `.lead`. Auf den meisten Handys (iPhone 14, Pro Max) bleibt der Kompass in voller Größe (320px); nur auf kurzen Bildschirmen (`@media (max-height:700px)`, z. B. iPhone SE) wird gezielt enger gestellt (Kompass 200px, kleinere Abstände/Titel), damit die Seite dort ohne Scrollen unter die Tab-Leiste passt.
