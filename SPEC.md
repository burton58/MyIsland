...
- Ausführlichere Erklärung in einer festen, hellen Kartenfläche (`.compass-explain-box`, gleiches Muster wie `.trial-box`/`.pay-note`) statt loser Text auf dem unscharfen Foto — deutlich besser lesbar als der text-shadow-Trick, den lose Textelemente auf dieser Seite sonst nutzen. Inhalt: kurzer Einleitungssatz ("Setze die Nadel dahin, wo du gerade stehst: zwischen Denken und Fühlen, und zwischen angespannt und entspannt."), je eine Zeile für Denken/Fühlen/Angespannt/Entspannt, Abschlusssatz ("Daraus finden wir deine passende Meditation."). Auf kleinen Bildschirmen (iPhone SE) ist die Karte dadurch enger gestellt (`#compassPage`-Overrides: kleinere Ränder/Schrift/Abstände, Kompass-Scheibe 222px statt 320px), damit die Seite weiterhin ohne Scrollen unter die Tab-Leiste passt.
