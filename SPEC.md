...
- Ausführlichere Erklärung: kurzer Einleitungssatz ("Setze die Nadel dahin, wo du gerade stehst: zwischen Denken und Fühlen, und zwischen angespannt und entspannt.") als normaler `.lead`-Text, darunter ein **2×2-Raster** (`.compass-explain-grid`) mit je einer kurzen Karte für Denken/Fühlen/Angespannt/Entspannt — spiegelt bewusst die zwei Achsen des Kompasses selbst. Die Karten sind leicht/glasig (`rgba(255,255,255,.55)` + `backdrop-filter:blur`), nicht solide wie `.trial-box`/`.pay-note` — eine erste solide Version wirkte wie eine separate Hinweisbox und passte nicht zur ruhigen, hellen Optik der restlichen Seite. Abschlusssatz ("Daraus finden wir deine passende Meditation.") wieder als `.lead`. Auf den meisten Handys (iPhone 14, Pro Max) bleibt der Kompass in voller Größe (320px); nur auf kurzen Bildschirmen (`@media (max-height:700px)`, z. B. iPhone SE) wird gezielt enger gestellt (Kompass 220px, kleinere Abstände), damit die Seite dort ohne Scrollen unter die Tab-Leiste passt.
