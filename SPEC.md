...
- Ausführlichere Erklärung: kurzer Einleitungssatz ("Setze die Nadel dahin, wo du gerade stehst: zwischen Denken und Fühlen, und zwischen angespannt und entspannt.") als normaler `.lead`-Text, darunter ein **2×2-Raster** (`.compass-explain-grid`) mit je einer kurzen Karte für Denken/Fühlen/Angespannt/Entspannt — spiegelt bewusst die zwei Achsen des Kompasses selbst. Die Karten haben volle Deckkraft (`#eef6f2`, gleicher Ton wie `.trial-box`) statt Transparenz — eine durchscheinende Glas-Variante (`rgba(255,255,255,.55)` + Blur) war je nach Fotostelle dahinter schlecht lesbar. Abschlusssatz ("Daraus finden wir deine passende Meditation.") wieder als `.lead`. Alle Abstände bleiben grosszügig (keine zusammengequetschten Ränder) — das wirkte in einer Zwischenversion billig. "Zurück"/"Weiter" sind bewusst **nicht** anders gestylt als auf jeder anderen Seite (unterschiedliche Knopfgrössen je Seite wirkten inkonsistent) — die nötige Höhe wird stattdessen beim Kompass eingespart, das fällt kaum auf: 200px auf normalen Handys (iPhone 14, Pro Max) statt dem App-Standard 320px, auf kurzen Bildschirmen (`@media (max-height:700px)`, z. B. iPhone SE) zusätzlich engere Abstände und ein noch kleinerer Kompass (150px). Mit dem zusätzlichen Dauer-Picker (s. u.) reicht das auf dem iPhone SE nicht mehr ganz für ein scrollfreies Layout — dort bewusst in Kauf genommen (kurzes Scrollen statt gequetschtem Layout), auf iPhone 14/Pro Max bleibt alles ohne Scrollen sichtbar.
