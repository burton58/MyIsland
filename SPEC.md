...
### 3.2 Kompass ("Wo stehst du gerade?")

Als einziger Screen bewusst nach einer **festen visuellen Hierarchie** aufgebaut (Redesign-Auftrag "Premium-Look, Kompass ist das Zentrum"): **1. Kompass → 2. Status-Badge → 3. Optionen → 4. Weiter-Button → 5. sekundäre Navigation**. Alle Abstände folgen einem **8-pt-Raster** (8/16/24/32).
...
- **Bühne (`.compass-stage`):** Titel, Hinweis, Kompass und Status-Badge liegen zusammen auf einer hellen, milchigen Fläche (`rgba(255,255,255,.55)` + `blur(20px) saturate(1.2)`, weicher weisser Rand, 32px Radius). Sie hebt den Kompass klar vom unruhigen Foto ab und ersetzt sowohl den früheren Text-Schein (`text-shadow`) als auch die Titel-Plakette — beides wird auf dieser Seite nicht mehr gebraucht, weil der Text jetzt auf einer eigenen Fläche sitzt.
- **Text kurz und aufgabenorientiert:** Überschrift **"Wo stehst du gerade?"** (Serif, 1.45rem) + eine Zeile **"Ziehe den Punkt auf dem Kompass."** Die früheren zwei langen Erklärsätze und die noch ältere 2×2-Erklär-Karten-Box (`.compass-explain-grid`) sind ersatzlos weg — sie haben den Kompass optisch verdrängt.
- **Kompass-Grösse:** 320px (Pro Max) bzw. breitenbegrenzt ~312px (iPhone 14) statt vorher 240px, auf kurzen Bildschirmen (`@media (max-height:700px)`, z. B. iPhone SE) 216px statt vorher 145px — also rund **+30 % bzw. +49 %**. Möglich wurde das durch den weggefallenen Erklärtext. Damit die Bühne auf schmalen Geräten die volle Breite nutzen kann, ist die seitliche Polsterung der `.compass-card` auf dieser Seite kleiner (8px statt 16px); die Luft sitzt stattdessen innen in der Bühne, wo sie sichtbar ist.
- **Status-Badge (`.mood-badge`)** direkt unter dem Kompass: ganzer Satz als Pille statt nur des Worts — z. B. "😊 Du fühlst dich ausgeglichen" (`moodBadgeHtml()`, siehe §5). Der Hinweis "Ziehe den Punkt, um es anzupassen" ist auf dieser Seite ausgeblendet (`#compassPage .compass-readout .sub{display:none}`), weil er als Zeile schon über dem Kompass steht; auf der Abschluss-Seite (§3.5) bleibt er sichtbar.
- **Anzahl- und Dauer-Picker für Meditation 2** (`#durationGroupCompass`, `.compass-prefs`): "Anzahl Meditationen" (1/2/3, `#countOptsV2`) und "Dauer" (5/10/15/20/30 Min, `#durationOptsV2`). Bewusst **untergeordnet gestaltet**: zwei gestapelte Zeilen (nicht mehr zwei Spalten, in denen die Dauer-Chips auf drei Zeilen umbrachen), kleine Grossbuchstaben-Labels, kompakte Chips mit `flex:1` — jede Gruppe passt so garantiert auf **eine** Zeile. Farben dezenter als die App-Standard-Chips (`rgba(255,255,255,.66)` statt `#f0f6f5`, aktiv weiches `#dcede4` mit Salbei-Rand statt kräftigem Teal). Setzen die globalen `desiredCountV2`/`durationV2`, die Meditation 2 (§3.3a) beim Öffnen einer Kategorie zum automatischen Auffüllen verwendet — beide Fragen kommen dadurch schon hier statt erst auf der Kategorie-Seite.
- **Aktionen:** "← Zurück" (sekundär) und der grüne CTA **"Zu den Meditationen →"**, beide unter den Optionen mit 24px Abstand. Der CTA wurde von "Weiter zu den Meditationen →" gekürzt, damit er auf **eine** Zeile passt; Grösse und Stil bleiben identisch zu den CTAs aller anderen Seiten (unterschiedliche Knopfgrössen je Seite wirkten inkonsistent).
- **Vertikale Zentrierung:** Auf hohen Geräten wird der Inhalt mittig gesetzt (`justify-content:center` auf `.compass-page`), sonst blieb unten ein grosses Loch. Überschreitet der Inhalt die Höhe, verhält sich die Seite wieder ganz normal.
- Weiterhin gilt: **alles inklusive der Buttons ganz unten bleibt ohne Scrollen sichtbar**, auf allen drei getesteten Bildschirmgrössen (375×667, 390×844, 430×932).
...
moodBadgeHtml(c)    // → ganze Pille "<emoji> Du fuehlst dich <wort>" (.mood-badge) - die grosse Anzeige
                        //   unter beiden Kompassen (Kompass-Seite §3.2 und Abschluss §3.5)
